...
class StoreService
{
  final String _url = 'http://localhost:8080/store';
  ...
  Future<List<Store>> getStoreList() async
  {
    ...
    if (response.statusCode == 200)
    {
      ...
      return (responseJson as List).map((p) => Store.fromJson(p)).toList();
    } else {
      throw Exception('Failed to load server data');
    }
  }
  ...
  {
    final response = await http.post(Uri.parse(_url), headers: headers, body: json.encode(store.toJson()));
    //статус 201 - создан
  ...
  Future<Store> putStore(Store store, int id) async
  {

    final response = await http.put(Uri.parse(_url + "/$id"), headers: headers, body: json.encode(store.toJson()));
